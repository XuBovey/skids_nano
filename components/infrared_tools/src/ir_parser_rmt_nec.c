...
#define NEC_DATA_FRAME_RMT_WORDS (30)
...
static bool nec_parse_connect(ir_parser_t *parser)
{
    nec_parser_t *nec_parser = __containerof(parser, nec_parser_t, parent);
    rmt_item32_t item = nec_parser->buffer[nec_parser->cursor];
    bool ret = (item.level0 == nec_parser->inverse) && (item.level1 != nec_parser->inverse) &&
...
static esp_err_t nec_parser_get_scan_code_ext(ir_parser_t *parser, uint8_t *data0, uint8_t *data1, uint8_t *data0_len, uint8_t *data1_len, uint8_t item_num)
{
    esp_err_t ret = ESP_FAIL;
    int i = 0;
    int i_tmp;
...
ir_parser_t *ir_parser_rmt_new_nec(const ir_parser_config_t *config)
{
    ...
    nec_parser->connect_code_high_ticks = (uint32_t)(ratio * NEC_CONNECT_CODE_HIGH_US);
    nec_parser->connect_code_low_ticks = (uint32_t)(ratio * NEC_CONNECT_CODE_LOW_US);
    nec_parser->ending_code_high_ticks = (uint32_t)(ratio * NEC_ENDING_CODE_HIGH_US);
    nec_parser->ending_code_low_ticks = 0x7FFF;
    ...
    nec_parser->parent.get_scan_code_ext = nec_parser_get_scan_code_ext;
