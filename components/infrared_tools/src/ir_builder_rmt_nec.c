...
typedef struct {
    ...
    uint32_t connect_code_high_ticks;
    uint32_t connect_code_low_ticks;
...
static esp_err_t nec_builder_make_connect(ir_builder_t *builder)
{
    nec_builder_t *nec_builder = __containerof(builder, nec_builder_t, parent);
    nec_builder->buffer[nec_builder->cursor].level0 = !nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration0 = nec_builder->connect_code_high_ticks;
    nec_builder->buffer[nec_builder->cursor].level1 = nec_builder->inverse;
    nec_builder->buffer[nec_builder->cursor].duration1 = nec_builder->connect_code_low_ticks;
    nec_builder->cursor += 1;
    return ESP_OK;
}

static esp_err_t nec_build_frame_ext(ir_builder_t *builder, uint8_t *ir_data0_buf, uint8_t * ir_data1_buf, uint16_t data0_bits_len, uint16_t data1_bits_len, uint8_t connect_en)
{
    builder->make_head(builder);

    // data0 LSB -> MSB
    for (int i = 0; i < data0_bits_len; i++) {
    	if (ir_data0_buf[i/8] & (1 << i%8)) {
			builder->make_logic1(builder);
		} else {
			builder->make_logic0(builder);
		}
    ...
    if(connect_en) {
    	nec_builder_make_connect(builder);
    }

    // data0 LSB -> MSB
    for (int i = 0; i < data1_bits_len; i++) {
    	if (ir_data1_buf[i/8] & (1 << i%8)) {
			builder->make_logic1(builder);
		} else {
			builder->make_logic0(builder);
		}
...
ir_builder_t *ir_builder_rmt_new_nec(const ir_builder_config_t *config)
{
    ...
    nec_builder->connect_code_high_ticks = (uint32_t)(ratio * NEC_CONNECT_CODE_HIGH_US);
    nec_builder->connect_code_low_ticks = (uint32_t)(ratio * NEC_CONNECT_CODE_LOW_US);
    ...
    nec_builder->parent.make_connect = nec_builder_make_connect;
    nec_builder->parent.make_end = nec_builder_make_end;
    nec_builder->parent.build_frame = nec_build_frame;
    nec_builder->parent.build_frame_ext = nec_build_frame_ext;
